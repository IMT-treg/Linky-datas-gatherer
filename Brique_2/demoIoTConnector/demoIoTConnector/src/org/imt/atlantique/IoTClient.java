...
import flo.teleinfo.MongoManager;
...
public class IoTClient implements CallBack, AccessPointListener {
	...
	public IoTClient() {
		 ...
		 * device or service.
		 */
		this.sensorDeviceAccessPoint = ubiStructFactory.newDeviceAccessPoint("linky",
...
//	}
	...
	public ArrayList<DataElement> receiveEventNotification(EventSubscription source, ParameterValue parameterValue) {
		...
		fillDatabase(parameterValue);
		return null;
	}
	//Florian PART
	public void fillDatabase(ParameterValue parameterValue) {
		database.docToDatabase(parameterValue);
	}
	...
	public static void viewParameterValue(ParameterValue parameterValue) {
		...
		for (Iterator attributeIterator = parameterValue.attributes(); attributeIterator.hasNext();) {
			...
			System.out.println("ATTRIBUT: " + attribut.getName().getLocalPart() + " : " + attribut.getValue());
		}
